    ...
    public partial class ucChoroplethMapItem : UserControl {
        ...
        void OnFileLoaded(object sender, ItemsLoadedEventArgs e) {
            foreach(var item in e.Items) {
                ...
                decimal value = 0; 
                if (item.Attributes["NAME"] !=null) value = SalesPerformanceDataGenerator.Current.TotalSales.Where(d => string.Equals(d.State, item.Attributes["NAME"].Value.ToString())).Sum(d => d.RevenueYTD);
                attr.Value = value;
                if(value == 0)
                    item.Visible = false;
