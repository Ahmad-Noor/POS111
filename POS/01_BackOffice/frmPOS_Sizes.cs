...
namespace POS
{
    public partial class frmPOS_Sizes : Sky_UserControl
    {
        public frmPOS_Sizes()
        {
        ...
        POS_Sizes mTable = new POS_Sizes();
        long ID;



        private void frmPOS_Sizes_Load(object sender, EventArgs e)
        {
            ...
            searchLookUpEdit1.Properties.DisplayMember = "SizeName";
            searchLookUpEdit1.Properties.View.Columns.AddField("ID").Visible = false;
            searchLookUpEdit1.Properties.View.Columns.AddField("SizeCode").Visible = true;
            searchLookUpEdit1.Properties.View.Columns.AddField("SizeName").Visible = true;
        ...
        void Filllookup()
        {var lookup = (from tb in db.POS_Sizes
        ...
        private void Retrieve(long id)
        {
            ...
            NewRecord = false;
            txtCode.Text = mTable.SizeCode;
            txtName.Text = mTable.SizeName;
        ...
        private void btnSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    mTable.SizeCode = txtCode.Text;
                    mTable.SizeName = txtName.Text;
        ...
        {
            var nav = db.POS_Sizes.Where(c => c.Branch_ID == Settings.CurrentBranch_ID).FirstOrDefault();
            if (nav == null) return;
            Retrieve(nav.ID);
        }
        ...
        private void btnNext_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (!NewRecord)
            {
                var nav = db.POS_Sizes.Where(c => c.ID > mTable.ID && c.Branch_ID == Settings.CurrentBranch_ID).FirstOrDefault();
                if (nav == null) return;
                Retrieve(nav.ID);
            }
        }
        ...
        private void btnPrevious_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (!NewRecord)
            {
                var nav = db.POS_Sizes.Where(c => c.ID < mTable.ID && c.Branch_ID == Settings.CurrentBranch_ID).OrderByDescending(c => c.ID).FirstOrDefault();
                if (nav == null) return;
                Retrieve(nav.ID);
            }
        }
        ...
        {
            var nav = db.POS_Sizes.Where(c => c.Branch_ID == Settings.CurrentBranch_ID).OrderByDescending(c => c.ID).FirstOrDefault();
            if (nav == null) return;
            Retrieve(nav.ID); 
        }
        ...
        {
            var validated = db.POS_Sizes.FirstOrDefault(c => c.SizeCode == txtCode.Text && c.Branch_ID == Settings.CurrentBranch_ID);
            if (validated != null)
            {
