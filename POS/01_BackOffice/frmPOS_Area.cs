...
namespace POS
{
    public partial class frmPOS_Area : Sky_UserControl
    {
        public frmPOS_Area()
        {
        ...
        POS_Area mTable = new POS_Area();
        long ID;



        private void frmPOS_Area_Load(object sender, EventArgs e)
        {
            ...
            searchLookUpEdit1.Properties.DisplayMember = "AreaName";
            searchLookUpEdit1.Properties.View.Columns.AddField("ID").Visible = false;
            searchLookUpEdit1.Properties.View.Columns.AddField("AreaCode").Visible = true;
        ...
        List<POS_Area> lookup;
        void Filllookup()
        {
           //var lookup = (from tb in db.POS_Area
           //               where tb.Branch_ID == Settings.CurrentBranch_ID
           //               select new { tb.AreaCode, tb.AreaName, tb.ID }).ToList();

          lookup = db.POS_Area.Where(c => c.Branch_ID == Settings.CurrentBranch_ID).ToList();


            searchLookUpEdit1.Properties.DataSource = lookup;
            dataNavigator1.DataSource = lookup;
        }
        ...
        private void Retrieve(long id)
        {
            ...
            int position = lookup.IndexOf(lookup.Find(c=>c.ID==id));
            if (position > -1)
                dataNavigator1.Position = position;



            mTable = db.POS_Area.Find(id);
            if (mTable == null)
            {
                btn_AddNew_ItemClick(null, null);
                return;
            ...
            NewRecord = false;
            txtCode.Text = mTable.AreaCode;
            txtName.Text = mTable.AreaName;
        ...
        private void btnSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    mTable.AreaCode = txtCode.Text;
                    mTable.AreaName = txtName.Text;
        ...
        {
            var nav = db.POS_Area.Where(c => c.Branch_ID == Settings.CurrentBranch_ID).FirstOrDefault();
            if (nav == null) return;
            Retrieve(nav.ID);
        }
        ...
        private void btnNext_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (!NewRecord)
            {
                var nav = db.POS_Area.Where(c => c.ID > mTable.ID && c.Branch_ID == Settings.CurrentBranch_ID).FirstOrDefault();
                if (nav == null) return;
                Retrieve(nav.ID);
            }
        }
        ...
        private void btnPrevious_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (!NewRecord)
            {
                var nav = db.POS_Area.Where(c => c.ID < mTable.ID && c.Branch_ID == Settings.CurrentBranch_ID).OrderByDescending(c => c.ID).FirstOrDefault();
                if (nav == null) return;
                Retrieve(nav.ID);
            }
        }
        ...
        {
            var nav = db.POS_Area.Where(c => c.Branch_ID == Settings.CurrentBranch_ID).OrderByDescending(c => c.ID).FirstOrDefault();
            if (nav == null) return;
            Retrieve(nav.ID); 
        }
        ...
        {
            var validated = db.POS_Area.FirstOrDefault(c => c.AreaCode == txtCode.Text && c.Branch_ID == Settings.CurrentBranch_ID);
            if (validated != null)
            {
