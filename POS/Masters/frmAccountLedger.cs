    ...
    public partial class frmAccountLedger : Form
    {
        ...
        public void SaveFunction()
        {
            try
            {
                ...
                if (isBankAccount)
                {
                    ...
                    infoAccountLedger.Branch_ID = txtBranch_ID.Text.Trim();
                }
                else
                {
                    if (isSundryDebtorOrCreditor)
                    {
                        ...
                        infoAccountLedger.Branch_ID = string.Empty;
                    }
                    else
                    {
        ...
        public void FillControls()
        {
            try
            {
                ...
                txtBranch_ID.Text = infoAccountLedger.Branch_ID;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (CheckUserPrivilege.PrivilegeCheck(PublicVariables._decCurrentUserId, this.Name, "Delete Item"))
                {
                    Delete();
                }
                else
        ...
        /// <summary>
        ///Enter key and backspace navigation in 'Branch_ID ' textbox keydown
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtBranch_ID_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                ...
                if (e.KeyCode == Keys.Back)
                {
                    if (txtBranch_ID.Text == string.Empty || txtBranch_ID.SelectionStart == 0)
                    {
                        txtBranchName.Focus();
                ...
                if (e.KeyCode == Keys.Back)
                {
                    if (txtMailingName.Text == string.Empty || txtMailingName.SelectionStart == 0)
                    {
                        if (isSundryDebtorOrCreditor)
                        {
                            tbctrlLedger.SelectedTab = tbMainDetails;
                            btnSave.Focus();
                            txtBranch_ID.SelectionStart = 0;
                            txtBranch_ID.SelectionLength = 0;
                        }
                    }
                }
            }
